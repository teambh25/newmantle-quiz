...
def recommend_answer():
    ...
    def generate_candidate():        
        ...
        def get_trends_words():
            @task
            def get_trends(): 
                trends = cg.fetch_google_trends(hours=72, num_news=3, limit=20)  # 3 days trends
                return trends
    ...
    def rec_n_days_answer(start_date: str, days: int, base_candidates: list):
        for d in range(days):
            ...
            use_special_candidate = special_candidates is not None and utils.with_prob(configs.SPECIAL_DAY_PROB)
